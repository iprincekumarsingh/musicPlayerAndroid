package com.codewizard.miter.coding_wizard_utils;
...
public class CustomArtistImageUtil {
    ...
    public void setCustomArtistImage(final Artist artist, Uri uri) {
        Glide.with(Coding_wizard_Config_file.getInstance())
                .load(uri)
                .asBitmap()
                ...
                .into(new SimpleTarget<Bitmap>() {
                    ...
                    public void onLoadFailed(Exception e, Drawable errorDrawable) {
                        ...
                        e.printStackTrace();
                        Toast.makeText(Coding_wizard_Config_file.getInstance(), e.toString(), Toast.LENGTH_LONG).show();
                    }
                    ...
                    public void onResourceReady(final Bitmap resource, GlideAnimation<? super Bitmap> glideAnimation) {
                        new AsyncTask<Void, Void, Void>() {
                            ...
                            protected Void doInBackground(Void... params) {
                                File dir = new File(Coding_wizard_Config_file.getInstance().getFilesDir(), FOLDER_NAME);
                                if (!dir.exists()) {
                                ...
                                if (succesful) {
                                    ...
                                    ArtistSignatureUtil.getInstance(Coding_wizard_Config_file.getInstance()).updateArtistSignature(artist.getName());
                                    Coding_wizard_Config_file.getInstance().getContentResolver().notifyChange(MediaStore.Audio.Artists.EXTERNAL_CONTENT_URI, null); // trigger media store changed to force artist image reload
                                }
                                return null;
                            }
                        }.execute();
